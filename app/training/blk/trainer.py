...
class Trainer:
    ...
    def evaluate_and_plot(self, model, X_test, y_test, sym_test, history, builder):
        ...
        # --- ПОИСК ОПТИМАЛЬНОГО ПОРОГА (Смещение к Recall) ---
        fpr_c, tpr_c, thresholds_c = roc_curve(y_test, y_prob_clean)
        j_scores = 1.5 * tpr_c - fpr_c # Штраф за пропуск Блокады ниже, вес Recall x1.5
        best_idx = np.argmax(j_scores)
        optimal_threshold = thresholds_c[best_idx]
        ...
        self._plot_binary_cm(y_test, y_pred_clean, title=f"Clean Test Data (Thr={optimal_threshold:.2f})", filename="cm_clean.png")
        self._plot_binary_cm(y_noisy, y_pred_noisy, title=f"Noisy Test Data (Thr={optimal_threshold:.2f})", filename="cm_noisy.png")
        ...
        groups_clean = self._compute_metrics_grouped(y_test, y_pred_clean)
        groups_noisy = self._compute_metrics_grouped(y_noisy, y_pred_noisy)
    ...
    def _compute_metrics_grouped(self, y_true, y_pred, n_groups=30):
        n = len(y_true)
        indices = np.arange(n)
    ...
    def _print_metrics(self, clean_groups, noisy_groups, auc_clean, auc_noisy):
        print("\n" + "="*70)
        print(f"ФИНАЛЬНЫЕ МЕТРИКИ: Normal vs Blockade (Канал: {self.channel_name})")
        print(f"Оптимальный порог, смещенный к Recall (меньше пропусков Блокад)")
        print("="*70)
        print(f"{'Метрика':<15} | {'Чистые данные':<25} | {'Зашумленные данные':<25}")
        print("-"*70)
        for metric in ['Accuracy', 'Recall', 'Specificity', 'F1-Score']:
            c_mean = np.mean(clean_groups[metric])
            c_ci = np.percentile(clean_groups[metric], [2.5, 97.5])
            n_mean = np.mean(noisy_groups[metric])
            n_ci = np.percentile(noisy_groups[metric], [2.5, 97.5])
    ...
    def _plot_roc(self, y_true, y_prob, threshold, auc, title, filename):
        ...
        # Красная точка на оптимальном пороге
        idx = np.argmin(np.abs(thresholds - threshold))
        plt.scatter(fpr[idx], tpr[idx], marker='o', color='red', s=100, label=f'Threshold = {threshold:.2f}')
