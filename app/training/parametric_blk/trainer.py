...
class ParametricBlkTrainer:
    def __init__(self, models_dir='models/parametric_blk', results_dir='results/parametric_blk'):
    ...
    def train(self, model, X_train, y_train, X_val, y_val, epochs=150, batch_size=128, lr=0.001, noise_std=0.15):
        ...
        for epoch in range(epochs):
            ...
            if v_loss < best_loss: 
                best_loss = v_loss; torch.save(model.state_dict(), os.path.join(self.models_dir, 'best_mlp_blk.pth')); patience_counter = 15
            else: 
                patience_counter -= 1
                if patience_counter == 0: break
                    
        model.load_state_dict(torch.load(os.path.join(self.models_dir, 'best_mlp_blk.pth')))
        model.eval()
        with torch.no_grad():
            val_logits = model(X_val.to(self.device))
            val_probs = torch.softmax(val_logits, dim=1).cpu().numpy()[:, 1]
        ...
        print(f"\n[INFO] Оптимальный порог на валидации: {self.optimal_threshold:.4f}")
    ...
    # НОВЫЙ МЕТОД: Бутстрап вместо разбиения на маленькие группы
    def _compute_metrics_bootstrap(self, y_true, y_pred, n_bootstraps=1000):
        ...
        for _ in range(n_bootstraps):
            # Сэмплируем с возвращением выборку РАВНОГО размера (1500 из 1500)
    ...
    def evaluate_and_plot(self, model, X_test, y_test, sym_test, X_noisy, y_noisy, sym_noisy):
        ...
        self._plot_boxplots(groups_clean, groups_noisy, title="Metrics CI (Normal vs Blockade)", filename="boxplots_binary.png")
    ...
    def _print_metrics(self, clean_groups, noisy_groups):
        print(f"\n{'='*70}\nФИНАЛЬНЫЕ МЕТРИКИ: Normal vs Blockade (Bootstrap 1000 CI)\n{'='*70}")
