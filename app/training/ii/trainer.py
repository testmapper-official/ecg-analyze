...
class Trainer:
    def __init__(self, channel_name='II', models_dir='models', results_dir='results'):
        ...
        # Создаем папки с учетом канала: models/II/, results/II/
    ...
    def train(self, model, X_train, y_train, X_val, y_val, epochs=100, batch_size=64, lr=0.0005):
        ...
        # === ТОТ САМЫЙ РАБОЧИЙ КОНФИГ ОБУЧЕНИЯ ===
        criterion = nn.CrossEntropyLoss(label_smoothing=0.05)
        optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-4)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=7)
        
        best_loss, patience_counter = float('inf'), 15
        ...
        for epoch in range(epochs):
            ...
            model.eval()
            with torch.no_grad():
                v_out = model(X_val.to(self.device))
                v_loss = criterion(v_out, torch.LongTensor(y_val).to(self.device)).item()
                v_acc = (v_out.argmax(1).cpu() == torch.LongTensor(y_val)).float().mean().item()

            history['loss'].append(t_loss/len(loader))
            history['acc'].append(t_corr/t_tot)
            history['val_loss'].append(v_loss)
            history['val_acc'].append(v_acc)
            
            scheduler.step(v_loss)
            
            # Имя модели теперь зависит от канала
    ...
    def evaluate_and_plot(self, model, X_test, y_test, sym_test, history, builder):
        ...
        with torch.no_grad():
            logits_clean = model(X_test.to(self.device))
        ...
        self._plot_binary_cm(y_test, y_pred_clean, title="Clean Test Data", filename="binary_cm_clean.png")
        self._plot_roc(y_test, y_prob_clean, title="ROC Curve: Clean Data", filename="roc_clean.png")

        # --- ОЦЕНКА НА ЗАШУМЛЕННЫХ ДАННЫХ ---
        print("\nГенерация зашумленной тестовой выборки (SNR -12 to 6 dB)...")
        ...
        self._plot_binary_cm(y_noisy, y_pred_noisy, title="Noisy Test Data", filename="binary_cm_noisy.png")
    ...
    def _plot_binary_cm(self, y_true, y_pred, title, filename):
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Normal', 'PSS'], yticklabels=['Normal', 'PSS'])
        plt.title(title)
